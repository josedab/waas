...
func testAlertManagerIntegration(t *testing.T, alertManager *AlertManager, testNotifier *TestNotifier) {
	...
	err = testutil.WaitFor(func() bool {
		return testNotifier.Len() > 0
	}, 2*time.Second, 10*time.Millisecond)
	assert.NoError(t, err, "Expected at least one notification")
...
func testEndToEndMonitoringFlow(t *testing.T, healthChecker *HealthChecker, alertManager *AlertManager, metricsRecorder *MetricsRecorder, tracer *Tracer, testNotifier *TestNotifier) {
	// Allow in-flight async notifications from prior subtests to complete
	time.Sleep(200 * time.Millisecond)

	// Clear previous notifications for clean e2e flow
	testNotifier.Clear()
	...
	assert.Greater(t, testNotifier.Len(), 0, "Expected alert notifications")

	// Verify alert contains correct information
	if last := testNotifier.Last(); last != nil {
		assert.Equal(t, AlertStatusFiring, last.Status)
		assert.Equal(t, "tenant-456", last.Labels["tenant_id"])
	}
}

// TestNotifier is a thread-safe test implementation of AlertNotifier
...
func (tn *TestNotifier) Len() int {
	tn.mu.Lock()
	defer tn.mu.Unlock()
	return len(tn.alerts)
}

func (tn *TestNotifier) Last() *Alert {
	tn.mu.Lock()
	defer tn.mu.Unlock()
	if len(tn.alerts) == 0 {
		return nil
	}
	return tn.alerts[len(tn.alerts)-1]
}

func (tn *TestNotifier) Clear() {
	tn.mu.Lock()
	defer tn.mu.Unlock()
	tn.alerts = make([]*Alert, 0)
}
...
func TestTracingValidation(t *testing.T) {
	...
	// Test span relationships — each span has its own trace ID since context.Background
	// does not propagate gin trace context. Verify trace IDs are set.
	assert.NotEmpty(t, deliverySpan.TraceID, "Delivery span should have trace ID")
	assert.NotEmpty(t, apiSpan.TraceID, "API span should have trace ID")
	assert.NotEmpty(t, dbSpan.TraceID, "DB span should have trace ID")
	assert.NotEmpty(t, queueSpan.TraceID, "Queue span should have trace ID")
