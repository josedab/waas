...
func testAlertRuleValidation(t *testing.T) {
	...
	// Alert history records all alerts — resolved alerts update status in place
	assert.Greater(t, len(history), 0, "Should have alert history entries")
...
func testAlertManagerStateValidation(t *testing.T) {
	...
	// Verify notifications were sent (async)
	_ = testutil.WaitFor(func() bool {
		return testNotifier.Count() > 0
	}, 2*time.Second, 10*time.Millisecond)
	assert.Greater(t, testNotifier.Count(), 0, "Expected alert notifications")
}
...
func testPerformanceMonitoringValidation(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		// Trigger some alerts for high queue depth (must match HighQueueDepth rule labels and threshold > 1000)
		if i > 50 {
			alertManager.EvaluateMetric("queue_depth", float64(i*100), map[string]string{
				"component": "queue",
				"queue":     "perf-queue",
			})
		}
	...
	_ = testutil.WaitFor(func() bool {
		return perfNotifier.Count() > 0
	}, 2*time.Second, 10*time.Millisecond)
	assert.Greater(t, perfNotifier.Count(), 0, "Expected performance alert notifications")
...
func (vtn *ValidationTestNotifier) Count() int {
	vtn.mu.Lock()
	defer vtn.mu.Unlock()
	return vtn.callCount
}
